...
df <- as.data.frame(table(todososdados01$TIPO_BOLSA))
df$porcentagem = round(df$Freq*100/sum(df$Freq),1)
rotulos<-paste("(",df$porcentagem,"%)",sep=" ")
colnames(df) <- c("Tipo_Bolsa", "Total", "Porcentagem")

## Etapa 01...
pie_tipo_bolsa <- ggplot(df, aes(x = "", y=Porcentagem, fill = factor(Tipo_Bolsa))) +
  ...
  labs(fill = "",
       x = NULL,
       y = NULL,
       title = "Tipo de Bolsa de 2005 à 2016",
       caption = "Fonte: http://dados.gov.br/dataset/mec-prouni") 

pie_tipo_bolsa + coord_polar(theta = "y", start = 0) +
  my_theme()
