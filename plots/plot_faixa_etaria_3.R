...
tabelaFaixaEtaria3 <- tabelaQuantidadePorAnoEFaixa %>%
...
grafico <- ggplot(tabelaFaixaEtaria3, aes(x = ANO_CONCESSAO_BOLSA, y = PERCENTUAL_REPRES)) +
    geom_bar(stat = "identity", width = 0.7, fill = "#006400") +
    labs(title = "% de participação da faixa etária (31 a 40 anos)",
         x = "Ano",
         y = "% de participação") +
