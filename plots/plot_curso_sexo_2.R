...
bolsistas_por_curso_sexo_2 <- prouni %>%
  filter(NOME_CURSO_BOLSA_NOVO %in% ("Administração")) %>%
  group_by_(.dots=c("SEXO_BENEFICIARIO_BOLSA", "ANO_CONCESSAO_BOLSA")) %>%
  summarize(total = n()) %>%
  as.data.frame()

plot_por_curso_sexo_2 <- ggplot() +
  geom_line(data = bolsistas_por_curso_sexo_2,
